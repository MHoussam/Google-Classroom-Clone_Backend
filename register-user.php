...
$email=$last_name=$first_name=$password=$occupation=$picture_path=$username="";
$response['status']="";
...
if(!empty($email) && !empty($username) && !empty($first_name) && !empty($last_name) && !empty($password)){
    ...
    $sql = $conn->prepare("insert into users (first_name,last_name,username,email,password,occupation,picture_path) values(?,?,?,?,?,?,?)");
    $sql->bind_param("sssssss", $first_name,$last_name,$username,$email,$hashed_password,$occupation,$picture_path);
    $sql->execute();
